...
*/
define('content_reveal_version','1.2.1');
...
function return_scr_start($heading="",$id="",$default="",$img_url="",$ext="") {
    ...
    if (!$error) {
        ...
        $img_code="<script type=\"text/javascript\">document.writeln('<img src=\"".$img_url."image".$image_num.".".$ext."\" class=\"scrimg\" id=\"scrimg_".$id."\" alt=\"Hide/reveal text\" title=\"Hide/reveal text\"/>');</script>";
        $heading = str_replace("%image%",$img_code,$heading);

        // Output JavaScript to content
        $output="<!-- Simple Content Reveal v".content_reveal_version." | http://www.artiss.co.uk/simple-content-reveal -->\n";
        $output.="<div class=\"scrhead\" onmouseover=\"document.body.style.cursor='pointer'\" onmouseout=\"document.body.style.cursor='default'\" onclick=\"swap_display('scrdiv_".$id."','scrimg_".$id."')\">\n";
        $output.=$heading."\n";
        $output.="</div>\n";
        $output.="<script type=\"text/javascript\">document.writeln('<div style=\"display: ".$default."\" class=\"scrdiv\" id=\"scrdiv_".$id."\">');</script>\n";
        return $output;
    }
}
