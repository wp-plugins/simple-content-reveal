...
function acr_start( $heading = '', $id = '', $default = '', $img_url = '', $ext = '', $cookie = '', $link = false, $title1 = '', $title2 = '' ) {
	...
	if ( $cookie == '' ) {
		...
		if ( $options[ 'cookies' ] == 1 ) {
			$cookie = $options[ 'time' ];
			if ( $options[ 'period' ] == 'd' ) {
				$cookie = $options[ 'time' ] * 24;
			}
		} else {
			$cookie = 0;
		}
	}
	if ( !is_numeric( $cookie ) ) { $error = report_acr_error( 'The cookie length was not numeric - it should be a number of hours', $plugin );  }

    // Validate ID and heading
    if ( $id == '' ) { $id = uniqid( 'ran_' ); }
    if ( $heading == '' ) { $error = report_acr_error( 'No heading was specified', $plugin ); }
	if ( $title2 == '' ) { $title2 = $title1; }

    // Set up and validate image extension
    $ext = strtolower( $ext );
    if ( $ext == '' ) { $ext = 'gif'; }
    if ( ( $ext != 'gif' ) && ( $ext != 'png' ) ) { $error = report_acr_error( 'Invalid image extension - must be PNG or GIF', $plugin ); }
	...
	if ( ( $default == 'hide' ) or ( $default == '' ) or ( $page_default == 'hide' ) ) {
		...
		if ( ( $default == 'show' ) or ( $page_default == 'show' ) ) {
			...
			$error = report_acr_error( "Invalid default - must be blank, 'show' or 'hide'", $plugin );
		}
	}
    ...
    if ( !$error ) {
		...
		if ( $page_default == 'off' ) {
			...
			if ( ( $page_default == 'hide' ) or ( $page_default == 'show' ) or ( $cookie == 0 ) ) {
				...
				$img_code .= "'+set_initial_img('" . $id . "','image" . $image_num . "')+'";
			}
			$img_code .= "." . $ext . "\" class=\"scrimg\" id=\"scrimg_" . $id . "\" ";
			if ( ( $page_default == 'hide' ) or ( $page_default == 'show' ) or ( $cookie == 0 ) ) {
				$img_code .= "alt=\"" . $alt_text . "\" title=\"" . $alt_text;
			} else {
				$img_code .= "alt=\"'+set_initial_alt('" . $id . "','" . $alt_text . "')+'\" title=\"'+set_initial_alt('" . $id . "','" . $alt_text . "')+'";
			}
			$img_code .= "\"/>');</script>";
		}
        ...
        $output = "<!-- " . $plugin . " v" . content_reveal_version . " | http://www.artiss.co.uk/simple-content-reveal -->\n";

		// Out the image information
		if ( $heading != 'noheading' ) {
			$output .= "<div class=\"scrhead\"";
			if ( $page_default  != 'off' ) {
				$output .= " onmouseover=\"document.body.style.cursor='pointer'\" onmouseout=\"document.body.style.cursor='default'\" onclick=\"swap_display('" . $id . "','" . $cookie . "','" . $title1 . "','" . $title2 . "')\"";
			}
			$output .= ">\n" . $heading . "\n";
			$output .= "</div>\n";
		}
		...
		if ( !$link ) {
			if ( $page_default != 'off' ) {
				...
				if ( ( $page_default == 'hide' ) or ( $page_default == 'show' )  or ( $cookie == 0 ) ) {
					...
					$output .= "'+set_initial_css('" . $id . "','" . $default . "')+'";
				}
...
* @since	2.0
*
* @param	string	$errorin		The error message
* @param	string	$plugin_name	The name of the plugin
* @return	string					Set to TRUE
*/

function report_acr_error( $errorin, $plugin_name ) {
